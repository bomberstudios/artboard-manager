...
import { getSettings } from './artboard-settings.js'
...
export function onDocumentChanged(context) {
  ...
  context.actionContext.forEach(change => {
    ...
    if (
      layer.type == 'Artboard' ||
      (layer.type == 'SymbolMaster' && config.arrangeSymbols)
    ) {
...
export function LayersMoved(context) {
  if (sketch.version.sketch < 59) {
    ...
    const movedLayers = Array.from(context.actionContext.layers).map(layer =>
      sketch.fromNative(layer)
    )
    if (
      movedLayers.filter(
        layer =>
...
export function Resize(context) {
  if (sketch.version.sketch < 59) {
    ...
    if (
      context.actionContext.name == 'NormalResize' ||
      context.actionContext.name == 'NormalMultipleResize' ||
      context.action == 'ResizeArtboardToFit.finish'
    ) {
      if (anArtboardIsSelected(context)) {
        ArrangeArtboards(context)
      }
...
export function manualArrangeArtboards(context) {
  ArrangeArtboards(context, true)
}

export function ArrangeArtboards(context, manualMode) {
  const autoMode = Settings.settingForKey('autoMode')
  if (autoMode == false && !manualMode) {
    return
  }
  ...
  // is disabled
  if (
    (page._object == symbolsPage && config.arrangeSymbolsPage == false) ||
    excludePage == true
  ) {
    return
  }
  ...
  var snapDistance =
    artboards.reduce((initial, artboard) => {
  ...
  var artboardRows = artboards
    .sort(sort_by_y_position)
    .reduce((initial, artboard) => {
  ...
  artboardRowValues.forEach(rowValue => {
    ...
    var artboardsInRow = artboards.filter(
      artboard => artboard.frame.y == rowValue
    )
    artboardsInRow.sort(sort_by_x_position).forEach((artboard, index) => {
      ...
      if (config.renameArtboards) {
        artboard.name =
          config.artboardBasenames[currentRow] +
          currentColumn.toLocaleString('en-US', {
            minimumIntegerDigits: Number(config.minimumIntegerDigits),
            useGrouping: false,
          })
      }
    })
...
export function checkMarkMenu() {
  ...
  let singlePluginsMenu = pluginsMenu
    .itemWithTitle('Artboard Manager')
    .submenu()
